      ...
      for (unsigned int A=0; A<nodes_per_elem; A++) { //Loop over nodes
		for(unsigned int i=0; i<dim; i++){ //Loop over nodal dofs
		  ...
		  /*//EDIT - You need to define Klocal here. Note that the indices of Klocal are the element dof numbers (0 through 23),
		    ...
		    //Klocal[3*A+i][3*B+k]=Klocal[3*A+i][3*B+k]+fe_values.shape_grad(3*A+i,q)[l]*C(i,k,j,l)*fe_values.shape_grad(3*B+k,q)[j]*fe_values.JxW(q);
        //Klocal[3*A+i][3*B+k]=Klocal[3*A+i][3*B+k]+fe_values.shape_grad(3*A+i,q)[l]*C(i,k,l,j)*fe_values.shape_grad(3*B+k,q)[j]*fe_values.JxW(q);
        Klocal[3*A+i][3*B+k]=Klocal[3*A+i][3*B+k]+fe_values.shape_grad(3*A+i,q)[l]*C(i,l,k,j)*fe_values.shape_grad(3*B+k,q)[j]*fe_values.JxW(q);
						}
	      			}
	    		}	
	  		}
        	...
        	for (unsigned int q=0; q<num_face_quad_pts; ++q){
                  ...
                  //std::cout << h << std::endl;
              	  for (unsigned int A=0; A<nodes_per_elem; A++){ //loop over all element nodes
              		...
              		Flocal[3*A+i]=Flocal[3*A+i]+h[i]*x*fe_face_values.shape_value(3*A+i,q)*fe_face_values.JxW(q);
